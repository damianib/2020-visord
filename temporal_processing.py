...
def frequential_filter_video(spatially_processed_frames: NumpyArray, lowcut: float, highcut: float, fs: float, order=5) -> NumpyArray:
    ...
    post_processed_video = np.zeros((total_time, height, width, 3))
    ...
    for color in range(3):
        for i in range(height):
            for j in range(width):
                for t in range(total_time):
                    post_processed_video[t][i][j][color] = pre_processed_video[color][i][j][t]
